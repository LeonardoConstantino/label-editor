...
`;function ne(e){return e===`success`||e===`error`||e===`warning`||e===`info`}var re=class extends HTMLElement{#e;#t=null;#n=null;#r=null;#i;#a;#o;#s;static observedAttributes=[`type`,`message`,`duration`];constructor(){super(),this.#e=this.attachShadow({mode:`open`}),this.#e.appendChild(te.content.cloneNode(!0)),this.#i=this.#e.querySelector(`[part="toast-root"]`),this.#a=this.#e.querySelector(`[part="toast-icon"]`),this.#o=this.#e.querySelector(`[part="toast-body"]`),this.#s=this.#e.querySelector(`[part="toast-bar"]`)}connectedCallback(){this.#t=new AbortController;let{signal:e}=this.#t;this.#d(this.getAttribute(`type`)),this.#f(this.getAttribute(`message`)??R.message),this.#p(),this.#m(),this.#e.querySelector(`[part="toast-close"]`).addEventListener(`click`,()=>{this.dismiss()},{signal:e}),requestAnimationFrame(()=>{this.setAttribute(`data-visible`,``)})}disconnectedCallback(){this.#t?.abort(),this.#n!==null&&clearTimeout(this.#n),this.#r?.cancel()}attributeChangedCallback(e,t,n){t===n||!this.isConnected||(e===`type`?this.#d(n):e===`message`?this.#f(n??R.message):e===`duration`&&(this.#r?.cancel(),this.#r=null,this.#p(),this.#m()))}dismiss(){this.removeAttribute(`data-visible`);let e=parseFloat(getComputedStyle(this).getPropertyValue(`--toast-duration-ms`).trim())||L;setTimeout(()=>{let e=new CustomEvent(`ui-toast:dismissed`,{bubbles:!0,composed:!0,detail:{type:this.#c,message:this.#l}});this.dispatchEvent(e),this.remove()},e)}get#c(){let e=this.getAttribute(`type`);return ne(e)?e:R.type}get#l(){return this.getAttribute(`message`)??R.message}get#u(){let e=parseInt(this.getAttribute(`duration`)??``,10);return Number.isFinite(e)&&e>=0?e:R.duration}#d(e){let t=ne(e)?e:R.type,n=ee[t];this.#a.innerHTML=z[t],this.#i.setAttribute(`role`,n),this.#i.setAttribute(`aria-live`,n===`alert`?`assertive`:`polite`)}#f(e){this.#o.textContent=e}#p(){let e=this.#u;if(e<=0){this.#s.style.display=`none`;return}this.#s.style.display=``,this.#r=this.#s.animate([{transform:`scaleX(1)`},{transform:`scaleX(0)`}],{duration:e,fill:`forwards`,easing:`linear`})}#m(){this.#n!==null&&(clearTimeout(this.#n),this.#n=null);let e=this.#u;e>0&&(this.#n=setTimeout(()=>{this.dismiss()},e))}};customElements.define(`ui-toast`,re);var ie=class e extends HTMLElement{#e;static MAX_TOASTS=5;constructor(){super(),this.#e=this.attachShadow({mode:`open`}),this.#e.innerHTML=`
    ...
    `,this.workspace.className=`canvas-workspace`,this.artboard.className=`label-artboard`,this.artboard.appendChild(this.canvas),this.workspace.appendChild(this.artboard),this.shadowRoot.appendChild(this.workspace))}setupListeners(){this.unsubscribe=r.on(`state:change`,e=>{this.redraw(e),this.updateWorkspaceVisuals(e)}),r.on(`request:canvas:snapshot`,e=>{e(this.ctx)}),r.on(`command:canvas:restore`,e=>{this.ctx.putImageData(e,0,0)}),this.canvas.addEventListener(`mousedown`,e=>this.handleMouseDown(e))}updateWorkspaceVisuals(e){this.workspace.style.backgroundImage=``}redraw(e=b.getState()){let t=e.currentLabel;if(!t)return;let{config:n,elements:r}=t,i=w.mmToPx(1,n.dpi)*n.previewScale;this.artboard.style.width=`${t.config.widthMM}mm`,this.artboard.style.height=`${t.config.heightMM}mm`,this.artboard.style.backgroundColor=n.backgroundColor||`#ffffff`,this.canvas.width=w.mmToPx(t.config.widthMM,n.dpi)*n.previewScale,this.canvas.height=w.mmToPx(t.config.heightMM,n.dpi)*n.previewScale,this.canvas.style.width=`100%`,this.canvas.style.height=`100%`,this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height),this.ctx.fillStyle=n.backgroundColor||`#ffffff`,this.ctx.fillRect(0,0,this.canvas.width,this.canvas.height),r.filter(e=>e.visible!==!1).sort((e,t)=>e.zIndex-t.zIndex).forEach(t=>{k.render(t,{ctx:this.ctx,scale:i,dpi:n.dpi}),e.selectedElementIds.includes(t.id)&&this.drawSelectionOutline(t,i,n.dpi)}),e.preferences.showGrid&&this.drawGridOverlay(e,i)}drawGridOverlay(e,t){let{gridSizeMM:n,gridColor:r,gridOpacity:i}=e.preferences,{widthMM:a,heightMM:o}=e.currentLabel.config,s=n*t;this.ctx.save(),this.ctx.beginPath(),this.ctx.strokeStyle=r||`rgba(99, 102, 241, 0.15)`,this.ctx.globalAlpha=i??.3,this.ctx.lineWidth=.5;for(let e=0;e<=a*t;e+=s)this.ctx.moveTo(e,0),this.ctx.lineTo(e,o*t);for(let e=0;e<=o*t;e+=s)this.ctx.moveTo(0,e),this.ctx.lineTo(a*t,e);this.ctx.stroke(),this.ctx.restore()}drawSelectionOutline(e,t,n){e.dimensions&&(this.ctx.save(),this.ctx.strokeStyle=`#6366f1`,this.ctx.lineWidth=w.ptToPx(this.selectionOutlineBorderWidth,n)*(t/w.mmToPx(1,n)),this.ctx.setLineDash([5,5]),this.ctx.strokeRect(e.position.x*t,e.position.y*t,e.dimensions.width*t,e.dimensions.height*t),this.ctx.restore())}handleMouseDown(e){let t=this.canvas.getBoundingClientRect(),n=(e.clientX-t.left)/t.width*this.canvas.width,i=(e.clientY-t.top)/t.height*this.canvas.height,a=b.getState();if(!a.currentLabel)return;let o=[...a.currentLabel.elements].sort((e,t)=>t.zIndex-e.zIndex).find(e=>k.hitTest(e,n,i,a.currentLabel.config));o?(r.emit(`element:select`,o.id),c.play(c.enumPresets.SELECT)):r.emit(`element:select`,[])}};customElements.define(`editor-canvas`,ue);var de=class extends HTMLElement{button;constructor(){super(),this.attachShadow({mode:`open`}),this.button=document.createElement(`button`)}static get observedAttributes(){return[`variant`,`disabled`]}connectedCallback(){this.setupBaseStyles(),this.render(),this.setupEvents()}attributeChangedCallback(){this.render()}setupBaseStyles(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${W}
...
`;var ge=new Set,_e=class extends HTMLElement{#e;#t;#n;#r=null;static observedAttributes=[`name`,`size`,`color`];constructor(){super(),this.#e=this.attachShadow({mode:`open`}),this.#e.appendChild(he.content.cloneNode(!0)),this.#t=this.#e.getElementById(`icon-container`),this.#n=this.#e.querySelector(`slot`)}connectedCallback(){this.#r=new AbortController;let{signal:e}=this.#r;this.#n.addEventListener(`slotchange`,()=>this.#s(),{signal:e}),this.#a(this.getAttribute(`size`)),this.#o(this.getAttribute(`color`)),this.#i(this.getAttribute(`name`)),this.#s()}disconnectedCallback(){this.#r?.abort()}attributeChangedCallback(e,t,n){if(t!==n)switch(e){case`name`:this.#i(n);break;case`size`:this.#a(n);break;case`color`:this.#o(n);break}}get name(){return this.getAttribute(`name`)}set name(e){e?this.setAttribute(`name`,e):this.removeAttribute(`name`)}get size(){return this.getAttribute(`size`)}set size(e){e?this.setAttribute(`size`,e):this.removeAttribute(`size`)}get color(){return this.getAttribute(`color`)}set color(e){e?this.setAttribute(`color`,e):this.removeAttribute(`color`)}static get catalog(){return Object.keys(G)}#i(e){if(this.#t.innerHTML=``,!e)return;let t=me[e];if(!t){ge.has(e)||(console.warn(`[ui-icon] Ícone "${e}" não encontrado no catálogo. Use um <svg> via slot para ícones customizados.`),ge.add(e));return}let n=document.createElementNS(`http://www.w3.org/2000/svg`,`svg`);n.setAttribute(`part`,`svg`),n.setAttribute(`viewBox`,`0 0 24 24`),n.setAttribute(`xmlns`,`http://www.w3.org/2000/svg`),n.setAttribute(`fill`,`none`),n.setAttribute(`stroke`,`currentColor`),n.setAttribute(`aria-hidden`,`true`),n.innerHTML=t,this.#t.appendChild(n)}#a(e){let t=fe(e);this.style.setProperty(`--icon-size`,`${t}px`)}#o(e){let t=e||`currentColor`;this.style.setProperty(`--icon-color`,t)}#s(){let e=this.#n.assignedElements({flatten:!0}),t=e.length>0;this.#t.hidden=t,t&&e.forEach(e=>{e.setAttribute(`aria-hidden`,`true`),e.tagName?.toLowerCase()===`svg`&&!e.hasAttribute(`viewBox`)&&console.warn(`[ui-icon] SVG customizado sem atributo viewBox pode não escalar corretamente.`)}),this.dispatchEvent(new CustomEvent(`ui-icon:slotchange`,{detail:{hasSlotContent:t},bubbles:!0,composed:!0}))}};customElements.define(`ui-icon`,_e);var ve=[`top`,`bottom`,`left`,`right`],ye=new CSSStyleSheet;ye.replaceSync(`
  :host {
    position: fixed;
    z-index: 9999;
    max-width: 320px;
...
`;var we=class extends HTMLElement{#e;#t;#n;#r=null;static observedAttributes=[`trigger`,`placement`,`tooltip`,`tooltip-ref`,`variant`,`delay`,`offset`];constructor(){super(),this.#e=this.attachShadow({mode:`open`}),this.#e.appendChild(Ce.content.cloneNode(!0)),this.#t=this.#e.querySelector(`slot[name="target"]`),this.#n=this.#e.querySelector(`slot[name="content"]`)}connectedCallback(){this.#t.addEventListener(`slotchange`,()=>this.#i()),this.#i()}disconnectedCallback(){this.#r?.destroy()}attributeChangedCallback(e,t,n){t===n||!this.#r||([`tooltip`,`tooltip-ref`,`variant`].includes(e)?this.#r.updateConfig(this.#a()):this.#i())}#i(){let e=this.#t.assignedElements({flatten:!0});if(!e.length)return;this.#r?.destroy();let t=e[0];this.#r=new Se(t,this.#a())}#a(){let e=this.#n.assignedNodes({flatten:!0}),t=e.length>0?this.#o(e):void 0;return{text:this.getAttribute(`tooltip`)||void 0,templateRef:this.getAttribute(`tooltip-ref`)||void 0,contentSlot:t,placement:this.getAttribute(`placement`)||`top`,variant:this.getAttribute(`variant`)||void 0,delay:Number(this.getAttribute(`delay`)||200),offset:Number(this.getAttribute(`offset`)||8),triggers:(this.getAttribute(`trigger`)||`hover focus`).split(/\s+/)}}#o(e){let t=document.createDocumentFragment(),n=document.createElement(`div`),r=e=>{if(e.nodeType===Node.TEXT_NODE)return e.cloneNode(!1);if(e.nodeType===Node.ELEMENT_NODE){let t=e,n=document.createElement(t.nodeName.toLowerCase());for(let e of Array.from(t.attributes))n.setAttribute(e.name,e.value);for(let e of Array.from(t.childNodes))n.appendChild(r(e));return n}return e.cloneNode(!1)};e.forEach(e=>{n.appendChild(r(e))});let i=n.querySelector(`[class*="tooltip-header"], [class*="header"]`);return i&&i.classList.add(`tooltip-header`),n.querySelectorAll(`[class*="section-title"], [class*="title"]`).forEach(e=>{e.classList.add(`section-title`)}),n.querySelectorAll(`[class*="grid"]`).forEach(e=>{e.classList.add(`controls-grid`)}),n.querySelectorAll(`kbd, [class*="kbd"]`).forEach(e=>{e.classList.add(`kbd`)}),n.querySelectorAll(`[class*="desc"], [class*="text-muted"]`).forEach(e=>{e.classList.contains(`section-title`)||e.classList.add(`control-desc`)}),n.querySelectorAll(`[class*="note"], [class*="math"]`).forEach(e=>{e.classList.add(`math-note`)}),n.querySelectorAll(`[class*="mb-"], [class*="section"]`).forEach(e=>{e.classList.add(`tooltip-section`)}),t.appendChild(n),t}open(){this.#r?.open()}close(){this.#r?.close()}get isOpen(){return this.#r?.[`#isOpen`]??!1}};customElements.define(`ui-tooltip`,we);var Te=class extends HTMLElement{unsubscribe=null;constructor(){super(),this.attachShadow({mode:`open`})}connectedCallback(){this.render(),this.setupListeners()}disconnectedCallback(){this.unsubscribe&&this.unsubscribe()}setupListeners(){this.unsubscribe=r.on(`state:change`,e=>{let t=this.shadowRoot?.getElementById(`undo`),n=this.shadowRoot?.getElementById(`redo`);t&&t.toggleAttribute(`disabled`,!e.canUndo),n&&n.toggleAttribute(`disabled`,!e.canRedo)}),r.on(`command:toolbar:upload-image`,()=>{this.shadowRoot?.getElementById(`file-input`)?.click()})}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${W};
        :host {
          display: flex;
    ...
    `,this.attachEvents())}attachEvents(){let e=this.shadowRoot,t=e.getElementById(`file-input`);e.getElementById(`add-text`)?.addEventListener(`click`,()=>{r.emit(`element:add`,A.create(u.TEXT))}),e.getElementById(`add-rect`)?.addEventListener(`click`,()=>{r.emit(`element:add`,A.create(u.RECTANGLE))}),e.getElementById(`add-border`)?.addEventListener(`click`,()=>{r.emit(`element:add`,A.create(u.BORDER))}),e.getElementById(`add-image`)?.addEventListener(`click`,()=>t.click()),t?.addEventListener(`change`,async e=>{let t=e.target.files?.[0];if(!t)return;let{imageProcessor:n}=await y(async()=>{let{imageProcessor:e}=await import(`./imageProcessor-MCCMy5io.js`);return{imageProcessor:e}},__vite__mapDeps([3,2])),i=await n.process(t),a=b.getState().currentLabel.config;r.emit(`element:add`,A.create(u.IMAGE,{dimensions:{width:w.pxToMm(i.width,a.dpi),height:w.pxToMm(i.height,a.dpi)},src:i.src}))}),e.getElementById(`open-help`)?.addEventListener(`click`,()=>{r.emit(`ui:open:help`,{tab:`guide`});let t=e.getElementById(`open-help`);t&&t.classList.remove(`pulse-help`)}),e.getElementById(`undo`)?.addEventListener(`click`,()=>r.emit(`history:undo`)),e.getElementById(`redo`)?.addEventListener(`click`,()=>r.emit(`history:redo`)),e.getElementById(`save`)?.addEventListener(`click`,async()=>{let{templateManager:e}=await y(async()=>{let{templateManager:e}=await Promise.resolve().then(()=>j);return{templateManager:e}},void 0);await e.saveCurrentLabel(),r.emit(`notify`,{type:`success`,message:`Template salvo com sucesso!`})}),e.getElementById(`open-batch`)?.addEventListener(`click`,()=>{let e=document.getElementById(`batch-modal`);e&&e.setAttribute(`open`,``),c.play(c.enumPresets.OPEN)})}};customElements.define(`editor-toolbar`,Te);function q(e){if(!e)return``;let t=document.createElement(`p`);return t.textContent=e,t.innerHTML}var Ee=class extends HTMLElement{input;labelElement;constructor(){super(),this.attachShadow({mode:`open`}),this.labelElement=document.createElement(`label`),this.labelElement.className=`label-prism`,this.input=document.createElement(`input`),this.input.className=`input-prism`}static get observedAttributes(){return[`label`,`type`,`value`,`placeholder`]}connectedCallback(){this.setupBaseStyles(),this.render(),this.setupEvents()}attributeChangedCallback(e,t,n){t!==n&&this.updateValues()}setupBaseStyles(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${W}
        :host {
          display: block;
    ...
    `)}rebuildPanel(e,t,n){let r=this.shadowRoot?.getElementById(`panel-content`),i=this.shadowRoot?.getElementById(`panel-title`),a=this.shadowRoot?.getElementById(`unit-count`);!r||!e||(t?(i&&(i.textContent=`PROPERTIES`),a&&(a.textContent=`${e.elements.length} UNITS`),r.innerHTML=[...e.elements].sort((e,t)=>t.zIndex-e.zIndex).map(e=>this.renderCardHtml(e,e.id===t)).join(``)):(i&&(i.textContent=`LABEL SETUP`),a&&(a.textContent=`BLUEPRINT`),r.innerHTML=this.renderDocumentSetup(e,n||b.getState().preferences)),this.updateWarningVisuals())}renderDocumentSetup(e,t){let{widthMM:n,heightMM:r,backgroundColor:i}=e.config;return`
      ...
      <div class="row-ui">
        ...
        <app-input label="Paper" type="color" data-doc-prop="backgroundColor" value="${q(i||`#ffffff`)}" class="fixed-small"></app-input>
      </div>
      <div class="row-ui">
    ...
    `}renderCardHtml(e,t){let n=q(e.id);return`
      <div class="element-card ${t?`selected`:``}" data-id="${n}">
        <div class="card-header" data-action="select">
          <span class="type-tag">${q(e.type)}</span>
          <span class="layer-name" data-layer-id="${n}">${q(e.name||e.type)}</span>
    ...
    `}renderElementProperties(e){return`
      ...
      <div class="row-ui">
        <app-input label="Layer Name" data-prop="name" value="${q(e.name||``)}" style="flex:1"></app-input>
      </div>
    ...
    `}renderTypeSpecificFields(e){return Ae(e)?`
        ...
        <div class="row-ui">
          <app-input label="Content" data-prop="content" value="${q(e.content)}" style="flex:1"></app-input>
        </div>
        <div class="row-ui">
          <ui-number-scrubber label="Size" data-prop="fontSize" value="${e.fontSize}" min="1" max="200" step="1" unit="pt"></ui-number-scrubber>
          <app-input label="Color" type="color" data-prop="color" value="${q(e.color)}" class="fixed-small"></app-input>
        </div>
        <div class="row-ui">
          ...
          <app-input label="Weight" data-prop="fontWeight" value="${q(String(e.fontWeight))}" class="fixed-small"></app-input>
        </div>
      `:je(e)?`
        <span class="label-prism">Appearance</span>
        <div class="row-ui">
          <app-input label="Fill" type="color" data-prop="fillColor" value="${q(e.fillColor)}" style="flex:1"></app-input>
          <app-input label="Stroke" type="color" data-prop="strokeColor" value="${q(e.strokeColor)}" style="flex:1"></app-input>
        </div>
        <div class="row-ui">
      ...
      `:Me(e)?`
        ...
        <div class="row-ui">
          <app-input label="Fit Mode" data-prop="fit" value="${q(e.fit)}" style="flex:1"></app-input>
        </div>
      ...
      `:Ne(e)?`
        ...
        <div class="row-ui">
          <app-input label="Style" data-prop="style" value="${q(e.style)}" style="flex:1"></app-input>
          <app-input label="Color" type="color" data-prop="color" value="${q(e.color)}" class="fixed-small"></app-input>
        </div>
        <div class="row-ui">
...
`;var Re={danger:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round">
    ...
    `}updateSidebarUI(){let e=this.shadowRoot.getElementById(`sidebar`);e.innerHTML=`
    ...
    `}renderGrid(e){let t=this.shadowRoot.getElementById(`cartridge-grid`);t.innerHTML=`
      ...
      ${(this.filter===`recent`?[...this.templates].sort((e,t)=>t.updatedAt-e.updatedAt):this.templates).map(e=>this.renderCartridgeHtml(e)).join(``)}
    `,this.attachEvents(e)}renderCartridgeHtml(e){let t=ke(new Date(e.updatedAt).toISOString(),{isRelative:!0}),n=e.config.widthMM/e.config.heightMM;return`
    <div class="group relative bg-surface-solid border border-border-ui rounded-2xl overflow-hidden shadow-panel transition-all duration-300 ease-spring hover:border-accent-primary hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.6),0_0_15px_rgba(99,102,241,0.2)]" data-id="${e.id}">
        ...
        <div class="bg-black w-full h-44 flex items-center justify-center p-6 relative border-b border-border-ui overflow-hidden shadow-inner">
          ...
          <img src="${e.thumbnail||``}" 
              ...
              style="aspect-ratio: ${n}; max-width: 100%; max-height: 100%;" />
        ...
        <div class="p-4 bg-surface-solid">
          <h4 class="font-sans text-[13px] font-bold text-text-main truncate mb-2.5 group-hover:text-accent-primary transition-colors" title="${e.name}">
            ${e.name}
          </h4>
    ...
    `}attachEvents(e){let t=this.shadowRoot;t.querySelectorAll(`.filter-btn`).forEach(t=>{t.addEventListener(`click`,e=>{this.filter=e.currentTarget.dataset.filter||`all`,c.play(c.enumPresets.TAP),this.render()},{signal:e})}),t.getElementById(`btn-new-template`)?.addEventListener(`click`,()=>{N.createNewProject(),this.closeVault(),c.play(c.enumPresets.OPEN)},{signal:e}),t.addEventListener(`click`,async e=>{let t=e.target,n=t.closest(`.action-load`),r=t.closest(`.action-duplicate`),i=t.closest(`.action-delete`);if(n){let e=n.dataset.id;await N.loadTemplate(e),c.play(c.enumPresets.SUCCESS),this.closeVault()}if(r){let e=r.dataset.id;await N.duplicateTemplate(e),c.play(c.enumPresets.TAP),this.loadAndRender()}if(i){let e=i.dataset.id;if(await Ve.ask(`Excluir template?`,`Esta ação não pode ser desfeita.`,{variant:`danger`,confirmText:`Excluir definitivamente`,cancelText:`Cancelar`,countdown:3})){let n=t.closest(`.cartridge-card`);n&&n.classList.add(`fade-out`),setTimeout(async()=>{await N.deleteTemplate(e),c.play(c.enumPresets.DELETE),this.loadAndRender()},400)}}},{signal:e})}closeVault(){let e=document.getElementById(`vault-modal`);e&&e.removeAttribute(`open`)}};customElements.define(`vault-gallery`,He);var Ue=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`})}connectedCallback(){this.render(),this.setupEvents()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${W};
        :host { display: block; }
    ...
    `,this.renderShortcuts())}renderShortcuts(){let e=this.shadowRoot?.getElementById(`shortcuts-wrapper`);if(!e)return;let t=this.shortcuts;if(this.searchQuery&&(t=t.filter(e=>this.getDescription(e).toLowerCase().includes(this.searchQuery)||e.key.toLowerCase().includes(this.searchQuery))),this.categoryFilter&&(t=t.filter(e=>e.category.toLowerCase()===this.categoryFilter?.toLowerCase())),t.length===0){e.innerHTML=`<div class="text-center text-text-muted text-xs py-4 font-mono">Nenhum atalho encontrado.</div>`;return}if(this.variant===`slim`){e.innerHTML=t.map(e=>`
        <div class="flex items-center gap-3 p-1.5 hover:bg-white/5 rounded transition-colors">
          <div class="min-w-15 flex justify-end">${this.formatKey(e.key,e.type)}</div>
          <span class="text-[11px] text-text-muted truncate">${this.getDescription(e)}</span>
        </div>
          ...
          <div class="flex flex-col gap-3.5">
            ${t.map(e=>`
              <div class="flex items-center text-[12px] group">
                <span class="text-text-main group-hover:text-white transition-colors">${this.getDescription(e)}</span>
                <span class="dot-leader"></span>
                <span class="shrink-0 flex items-center justify-end">${this.formatKey(e.key,e.type)}</span>
              </div>
            `).join(``)}
          </div>

        </div>
      `;e.innerHTML=r}};customElements.define(`ui-keyboard-shortcuts`,We);var Ge=class extends HTMLElement{activeTab=`guide`;constructor(){super(),this.attachShadow({mode:`open`})}connectedCallback(){this.render()}setTab(e){this.activeTab=e,this.render()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${W};
        :host { display: block; height: 100%; }
        .tab-btn.active {
    ...
    `}setupEvents(){let e=this.shadowRoot;if(e.querySelectorAll(`[data-tab]`).forEach(e=>{e.addEventListener(`click`,e=>{let t=e.target.dataset.tab;t!==this.activeTab&&(this.activeTab=t,c.play(c.enumPresets.TAP),this.render())})}),this.activeTab===`shortcuts`){let t=e.getElementById(`shortcut-viewer`);t&&(t.data=I.listShortcuts())}}};customElements.define(`ui-help-center`,Ge);var Ke=[`default`,`success`,`danger`,`warning`,`info`],qe=[`sm`,`md`,`lg`,`xl`,`fullscreen`],Je=[`fade`,`scale`,`slide`,`flip`],Ye={default:{accent:`#6366f1`,fg:`#fff`,icon:null},success:{accent:`#16a34a`,fg:`#fff`,icon:`✓`},danger:{accent:`#dc2626`,fg:`#fff`,icon:`!`},warning:{accent:`#d97706`,fg:`#fff`,icon:`⚠`},info:{accent:`#0284c7`,fg:`#fff`,icon:`i`}},Xe={sm:`28rem`,md:`40rem`,lg:`56rem`,xl:`72rem`,fullscreen:`100vw`};function Y(e){return Ke.includes(e)}function X(e){return qe.includes(e)}function Z(e){return Je.includes(e)}var Ze=`
  :host {
    display: contents;
      ...
      <div class="flex flex-col gap-3 animate-in fade-in duration-500">
        ...
        <div class="overflow-x-auto border border-border-ui/30 rounded-lg">
          <table class="data-mini-table">
            <thead>
              <tr>${n.map(e=>`<th>${e}</th>`).join(``)}${t.length>4?`<th>...</th>`:``}</tr>
            </thead>
            <tbody>
